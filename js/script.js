$(document).ready(function(){
    ...
    //var city = "Seoul"; //내가 선택한 버튼의 날씨를 알려주기 위한 도시명을 넣을 초기 변수를 구성한다.
    ...
    배열 변수명.push(b) : ["a"] => ["a", "b"]; --> 마지막 인덱스에 새로운 데이터인 "b"를 넣겠다.
    shift() : 첫번째 인덱스 데이터를 제거한다.
    unshift("새로운 데이터") : 첫번째 인덱스 자리에 새로운 데이터를 추가한다. --> 전체적인 인덱스 번호가 하나씩 뒤로 밀린다.
    */

   var myKey = "f6bf9e717d25b3b30acb0809da69fd8a"; //사이트로부터 받아온 API Key를 저장.

   var state_icon = ""; //날씨 아이콘의 이름을 초기 변수로 구성한다.
   ...
   //["서울"] -->  <li>구조를 한번 돌린 상태, <ul>아래 존재하는 <li>태그는 1개.
   //["서울", "부산"] -->  <li>구조를 두번 돌린 상태, <ul>아래 존재하는 <li>태그는 3개.

    function w_info(){

        $("#weather ul").empty();

        //배열 데이터의 수를 기준으로 다시 반복하여 쌓는다.
        ...
        $("#weather ul li").each(function(index){ //0, 1, 2, ...
            $.ajax({
                ...
                success : function(data){
                    ...
                    console.log("현재 구름의 양(%) : " + data.clouds.all);
                    var cloud = data.clouds.all;

                    //텍스트(weather의 데이터 : clear, rain, snow)로 받아온 현재 날씨를 이미지 아이콘으로 변경 -> 클래스명을 추가(wi wi-...)
                    if(weather == "Clear"){
    ...
    $(".cities button").click(function(){
        //city = $(this).text(); //기존 전역변수인 var city에 새로운 값                                      을 대입하겠다는 의미.
        ...
        w_info(); //함수문으로 접근시켜라. (버튼을 클릭했을때)
    });


    function search(){
        
        var $search_val = $("#search_box").val(); //입력 상자에 사용자가 입력한 문자형 데이터를 받아오겠다.
        if($search_val.length<1){ //입력값이 없을 경우
            alert("검색어를 입력하세요!");
        }else{
    ...
    $(".search").keypress(function(event){
        console.log(event);
        if(event.keyCode == 13){ //입력 후 엔터값을 눌렀을때
            search();
        };
    });
...
//api키: f6bf9e717d25b3b30acb0809da69fd8a
//api.openweathermap.org/data/2.5/weather?q=London&appid=f6bf9e717d25b3b30acb0809da69fd8a
